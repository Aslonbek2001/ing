...
from menus.serializers.postgraduate_education_serializers import (
    PostgraduateEducationListSerializer,
    PostgraduateEducationSerializer,
)


@extend_schema(
    tags=["Postgraduate Education"],
...
class PostgraduateEducationListCreateAPIView(generics.ListCreateAPIView):
    queryset = Page.objects.filter(type="postgraduate_education").order_by("position")
...
class PostgraduateEducationDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    serializer_class = PostgraduateEducationSerializer
    permission_classes = [IsAuthenticatedOrReadOnly]
    lookup_field = "lookup"
